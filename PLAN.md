...
- [ ] **DEFECT (user, live, 2026-07-11): app bar appeared over the user's fullscreen game during a harness trace run.** Root cause: trace profiles set the host-active override (bypasses SR-LIFE gating for testability) and the app-bar/overlay windows are WS_EX_TOPMOST → they paint above whatever the user is doing while the harness borrows the desktop. Fix (harness etiquette): (a) harness_driver.py refuses to start a run while the foreground window is a fullscreen non-PowerPoint app (waits + logs); (b) under harness override, create overlay/app-bar windows NOTOPMOST so they never cover a foreground fullscreen app; verify captures still valid

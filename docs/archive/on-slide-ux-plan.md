...
1. **Slide View Subclassing**: Intercept Win32 window messages (`WM_MOUSEMOVE`, `WM_LBUTTONDOWN`, `WM_RBUTTONDOWN`, `WM_LBUTTONDBLCLK`) on PowerPointÔÇÖs document window to track mouse states and trigger contextual toolbars without breaking PowerPoint's native selection.
...
4. **Undo Stack Preservation**: Modifying a task on-slide and reflowing must be represented as a single cohesive action on PowerPointÔÇÖs native undo-redo transaction layer, avoiding fragmentation.

---

## 4. V2 ÔÇö Pure On-Slide Editor (Interaction Capture)
...
> mouse-down lands on OUR overlay we receive real `WM_MOUSEMOVE` ÔÇö drags are
...
- Click a bar ÔåÆ *our* selection chrome (not PowerPoint's), semantic object = task.
- Drag bar body ÔåÆ move task (day snapping, live ghost + date tooltip).
- Drag bar edge ÔåÆ resize (change start/end independently).
- Drag bar vertically ÔåÆ reassign row.
- Drag on empty row space ÔåÆ phantom bar ÔåÆ create task.
- Double-click bar/label/title ÔåÆ floating inline editor (label, start/end dates, %, color).
- Right-click ÔåÆ our own popup menu (TrackPopupMenu on the overlay ÔÇö PowerPoint's
...
render the chart as a single picture ÔÇö bulletproof selection-blocking but kills
...
|---|---|---|---|
| U1 | `capture-surface` ÔÇö whole CHART_ROOT area returns HTCLIENT; internal semantic hit-testing (bars, edges ┬▒4px, milestones, labels, row bands, empty cells) computed headlessly from PP_DOC + layout. Extend PP_PROJ with y-projection (row tops/heights) so hit-testing never needs COM. Escape hatch: Alt+click ÔåÆ HTTRANSPARENT pass-through (select/move whole group); small "move chart" grip in chrome. | ÔÇö | hit-test unit tests in ops harness (pure, COM-free); manual: click on chart never selects a child shape |
| U2 | `selection-suppression` ÔÇö Tick watches PowerPoint selection; if a PP_KIND child (not the group) becomes selected via any path we can't intercept (Tab-cycling, Selection Pane, marquee started outside the chart), unselect within one tick and mirror to our selection model. | ÔÇö | harness: programmatically select a child shape ÔåÆ cleared Ôëñ200ms; normal shapes outside chart unaffected |
| U3 | `alpha-overlay` ÔÇö migrate paint path from magenta color-key to `UpdateLayeredWindow` with per-pixel premultiplied alpha (GDI+ back buffer). Unlocks translucent hover bands, ghost bars, soft selection chrome. | ÔÇö | overlay harness PNG shows real translucency; no magenta fringing; existing chrome renders identically or better |
| U4 | `own-selection-model` ÔÇö internal selection state (task/milestone/row/none) driven by our hit-testing; selection chrome (frame, handles, badge) rendered on our surface; Esc clears. Keyboard (Delete, arrows) deferred to U9 pending focus-strategy discovery. | U1, U3 | manual + harness screenshot: clicking a bar shows our chrome, PowerPoint selection stays empty |
| U5 | `drag-move-resize` ÔÇö mouse-down on bar/edge starts modal drag (SetCapture on overlay): ghost bar + live date tooltip, snap to whole days via PP_PROJ inverse; drop ÔåÆ GanttOps mutation ÔåÆ rebuild. Suppress Tick/auto-reflow during gesture (g_mutating). | U1, U3, U4 | SendInput-driven harness: drag bar +N px ÔåÆ doc dates shift exactly N/ptPerDay days; REFLOW PASS still green |
| U6 | `drag-row-and-create` ÔÇö vertical drag reassigns row (`MoveTaskToRow` op exists); drag on empty slot draws phantom bar and creates a task on drop. | U5 | SendInput harness: vertical drag changes rowId; empty-space drag creates task with correct row+dates |
| U7 | `rebuild-in-place` ÔÇö replace delete-group+re-emit with a diff pass: move/resize/retitle existing shapes by PP_ID, add/remove only deltas. Wrap each gesture with `Application.StartNewUndoEntry` so one gesture Ôëê one undo step. Kills flicker, selection churn, and undo fragmentation. | U5 | harness: one drag gesture ÔåÆ single undo entry restores prior state; no shape-count churn on no-op |
| U8 | `floating-editor` ÔÇö double-click task/milestone ÔåÆ floating card editor (label, start date, end date, %, color) using the focusable top-level window pattern from V1 inline-edit; Enter commits, Esc cancels. Covers task-bar labels (V1 gap). | U1, U4 | harness or manual: edit start date via editor ÔåÆ doc + shapes update; Esc leaves untouched |
| U9 | `keyboard-and-cursors` ÔÇö per-zone cursors (move/ew-resize/ns-resize/crosshair); keyboard Delete/arrow-nudge for our selection (discovery: WH_KEYBOARD hook vs focusable overlay mode ÔÇö decide, then implement). | U4 | manual checklist; no interference with PowerPoint typing outside chart |
| U10 | `dpi-and-monitors` ÔÇö make the ADD-IN DPI-aware (V1 gap: only the test harness calls SetProcessDPIAware); verify overlayÔåöchart alignment at 100/125/150/200% and across monitors. | U3 | manual matrix at 4 scale factors; document results in this file |

Regression gates for every unit: `native\build.bat` [build] OK ┬À
`build-conformance.bat` fixtures pass ┬À `build-ops.bat` OPS HARNESS OK ┬À
`build-reflow.bat` + `ppreflow.exe` REFLOW PASS.

#### 4.2.1 U10 manual verification matrix (DPI ├ù monitor)
...
PowerPoint's DPI context ÔÇö the overlay's chart-alignment was never the risk.
...
This is user visual feedback, not a hard CI gate ÔÇö run it opportunistically
across scale-factor/monitor changes, not on every commit.

Checklist ÔÇö for each of the 4 Windows scale factors (100%, 125%, 150%, 200%),
...
|---|---|
| **OverlayÔåöchart alignment** | Selection frame, badge, and toolbar sit exactly over the CHART_ROOT shape with no visible offset/drift; drag the PowerPoint window between monitors of different scale and confirm the overlay follows without a lag/misalignment frame. |
...
Results (fill in per run ÔÇö date, scale factors covered, monitor config, pass/fail + notes):

| date | scale factor(s) | monitor config | result | notes |
|---|---|---|---|---|
| _(pending)_ | 100/125/150/200% | _(pending)_ | _(pending)_ | _(pending ÔÇö run manually per the checklist above and record here)_ |
...
3. **Keyboard focus on a NOACTIVATE overlay** is unsolved (V1 finding) ÔÇö U9 starts
   with a discovery spike, not implementation.
4. **Z-order**: overlay vs PowerPoint dialogs/flyouts ÔÇö verify menus/dialogs still
...
## 6. V3 ÔÇö Row-Centric Editing + Bottom App Bar

> Status: PLANNED (user feedback session 2026-07-04). Builds strictly on the V2
> interaction layer (overlay capture, own selection, drag engine, GanttOps ÔåÆ
> RebuildChart, one-gesture-one-undo, shared harness).

### 6.1 Vision (user feedback distilled)

1. **Fit-to-slide by default** ÔÇö a newly inserted chart fills the slide's content
   area, leaving the top ~15% free for a native PowerPoint title placeholder.
2. **Everything is a row** ÔÇö no separate "category" concept in the UX. Every row is
   ...
   way ÔÇö `PpRow.groupId` ÔÇö this is a UX unification, not a data migration.)
3. **Left rail as a first-class label surface** ÔÇö per-task label placement:
   on-bar, in the left rail, or both. Rail width becomes a real layout input.
4. **Generic movable markers** ÔÇö every vertical marker (Today, deadlines, custom)
   is draggable and label-editable; more can be added. "Today" is just a default label.
5. **Text elements** ÔÇö new `PpText` model element; free-floating or anchored to a
   task/milestone (`anchorId`), so anchored text follows its anchor on every rebuild.
6. **Grid + scale controls** ÔÇö doc-level timescale-grid visibility/style options
   alongside the existing D/W/M scale ops.
7. **Bottom app bar** (PowerNote `BottomToolbar` pattern) ÔÇö a docked contextual
...
8. **Dependency UX** ÔÇö create/remove dependencies from the app bar (link mode:
...
shared command map) ┬À uniform rows with optional hierarchy (keep `groupId`) ┬À
generic movable markers ┬À fit-to-slide reserves a top title zone.

### 6.2 Work units (dependency-ordered)

| id | unit | depends on | gate |
|---|---|---|---|
| V3-1 | `fit-to-slide` ÔÇö InsertGantt sizes/positions CHART_ROOT to the slide content area below a reserved title zone (top ~15%). Approach: build as today, then set group frame, then ReflowFromSlide re-projects (pure layout + conformance untouched). | ÔÇö | reflow harness asserts inserted frame Ôëê content area ÔåÆ `FIT OK` |
| V3-2 | `marker-model-ops` ÔÇö pure ops: `AddMarker`, `SetMarkerDate`, `SetMarkerLabel`, delete via existing `DeleteById`; marker type `custom` allowed. | ÔÇö | ops harness `MARKER OPS OK` |
| V3-3 | `marker-drag` ÔÇö hit-test zone for vertical markers (┬▒band, DPI-scaled), ew-resize cursor, modal drag with ghost line + date tooltip, commit `SetMarkerDate`. | V3-2 | overlay harness stage `MARKERDRAG PASS` |
| V3-4 | `text-model` ÔÇö `PpText` {id,label,anchorId,rowId,date,dx,dy}; JSON round-trip; layout (anchored ÔåÆ relative to anchor + offset; free ÔåÆ row+date); emitter (`PP_KIND=TEXT`); ops Add/SetLabel/Move/Delete. | ÔÇö | `TEXT OPS OK` + conformance fixtures pass |
| V3-5 | `text-interaction` ÔÇö hit zone Text, select/drag (anchored drag adjusts dx/dy; free drag moves row/date), double-click reuses card editor, Delete key works. | V3-4 | overlay harness stage `TEXT PASS` |
| V3-6 | `label-placement` ÔÇö per-task `labelPlacement` (bar\|rail\|both, default bar); rail renders task labels for rail/both; rail width a layout input; `SetLabelPlacement` op. | ÔÇö | `LABEL OPS OK` + conformance |
| V3-7 | `row-uniform-ux` ÔÇö ops `AddRowAbove`, `IndentRow`, `OutdentRow` (via groupId, one level); uniform row treatment in hit zones/menus. | ÔÇö | `ROW OPS OK` |
| V3-8 | `grid-scale-options` ÔÇö all FIVE scales become real (year\|quarter\|month\|week\|day drive a two-tier axis: primary band + separator ticks, e.g. week ÔåÆ months + Monday separators, quarter ÔåÆ years + Q1..Q4); separator density override `gridDensity` (auto\|year\|quarter\|month\|week\|day\|none) + `gridStyle` (solid\|dotted); `SetGridDensity`/`SetGridStyle` ops; separator cap ~150 with coarser fallback. Conformance fixtures are a layout-only cross-impl contract with the web TS engine ÔÇö never regenerated. | ÔÇö | `GRID OPS OK` + full suite |
| V3-9 | `appbar-shell` ÔÇö second docked layered NOACTIVATE strip at the bottom of the slide area; PURE app-bar model `BuildAppBar(selection, doc-state) ÔåÆ sections/buttons` (COM-free, ops-testable); GDI+ paint (shared Material palette), hover, click ÔåÆ command ids. Shows when a chart is on the active slide; obeys the same foreground scoping as the chart overlay (V3-13). | V3-2, V3-4, V3-6, V3-7, V3-8, V3-13, V3-14 | ops `APPBAR MODEL OK`; overlay harness stage `APPBAR PASS` |
| V3-10 | `appbar-actions` ÔÇö ONE shared command registry (refactor menu `MapMenuCommand` into it); wire every app-bar button per context: task (nudge/dates/percent/color/placement/add-text/delete), milestone, row (add/indent/outdent/delete), marker (date/label/delete), background (scale, grid, add row/marker/text). | V3-9 | overlay harness stage `APPBARACT PASS` (button click ÔåÆ doc mutation) |
| V3-11 | `dependency-ux` ÔÇö app bar Link button ÔåÆ link mode (crosshair + banner), click target task ÔåÆ `AddDependency` (finish-to-start default), Esc cancels; Unlink removes deps touching selection. | V3-10 | `DEP OPS OK`; overlay harness stage `DEP PASS` |
| V3-12 | `context-menu-v3` ÔÇö right-click menus rebuilt FROM the shared command registry: add milestone (new `AddMilestone` op)/text/marker, grid + scale submenus (all five scales), link mode entry. | V3-10 | new marker `MENU MAP V3 OK`; all harness stages green |
| V3-13 | `fix-overlay-scoping` ÔÇö **user bug 2026-07-04**: overlay chrome floats above OTHER apps (seen over CurseForge) because the TOPMOST overlay is only hidden when no chart exists, never on foreground/iconic checks. Fix: Tick hides all our windows (overlay, editor, card) unless PowerPoint is foreground (or the foreground window is ours / a POWERPNT-process dialog) and not minimized. Harness must explicitly foreground PowerPoint before overlay stages. | ÔÇö | overlay harness stage `SCOPE PASS` |
| V3-14 | `material-theme` ÔÇö real Material Design pass: shared Material-3 tonal palette in a new pure `GanttTheme.h` consumed by BOTH the shape emitter (bars, rail, header, gridlines, markers, deps) and the overlay chrome (selection, toolbar, ghost, tooltip, card editor); modern look matching the web app's feel with a better palette. Slide-export PNG artifact for visual inspection. | V3-8, V3-13 | new marker `THEME PNG OK` + full suite + fresh `native/build/material-theme-preview.png` |

Regression gates for every unit (same suite as V2): `native\build.bat` [build] OK ┬À
`build-ops.bat` all markers ┬À `build-conformance.bat` fixtures pass ┬À
`build-overlay.bat` + `ppoverlay.exe` all stages exit 0 ┬À
...
1. **Two overlay windows** (chart overlay + app bar) must never fight for capture;
   the app bar is outside the chart rect so NCHITTEST domains are disjoint ÔÇö verify.
...
5. **Grid options widen PP_DOC schema**: keep JSON round-trip backward-compatible
   (absent fields ÔåÆ current defaults) so existing decks load unchanged.

...
### Log (session continues, 2026-07-12)
- U2 committed e7fdefb after 2 agent rounds + coordinator forensics: cursor/composer-2.5 delivered the features but 3 compile errors (decl order x2, group-scope decl) and one DESIGN overreach (unit-snapped task drags -> INPLACE e2e caught it; snap now markers-only). Coordinator found 2 deep engine bugs the agents missed: (a) write-only group frame re-pin SCALES children (PPT semantics) -> bounds-changing ops now fall back to full reconcile; (b) drag-commit echo pixels persisted on the paint-free idle overlay -> clear now repaints.
- U3 committed ccd2c42 after 1 agent round + coordinator root-cause: cursor implemented docking/context purity correctly BUT the verification captures showed mixed chrome. Coordinator instrumentation proved TWO app-bar windows: the REGISTERED add-in inside the harness PowerPoint ran its own overlay exactly overlapping the harness's (the documented add-in-inside-harness gotcha, made visible by the NOTOPMOST etiquette change). Fix: PP_HARNESS presentation tag + add-in stand-down + PrintWindow captures. 2 cursor spawns died silently this stretch (known ~30% pathology).
- Provider notes: cursor/composer-2.5 = fast, high feature throughput, ~1 compile error per round, never has a shell, twice fabricated verification claims (rounds 3, and premature PLAN ticks in U3); copilot/claude-opus-4.8 = only agent that runs its own edit->build->trace loop (U1, walkthrough runner) but hit monthly quota; codex/gpt-5.5 = flawless on doc migrations, honest, fast; grok-build = 1 excellent SRS but 50% timeout/death rate.
